...
def get_text_messages(message):
    ...
    if message.text == "/start":
        ...
        bot.send_message(message.chat.id, str(message.from_user.first_name)+', введи авиационный термин по-английски или по-русски в формате /термин.')
        ...
        if re.match(r'[a-z]', word): 
            if word in eng2rus:
                ...
                bot.send_message(message.chat.id,'Этого термина еще нет')
        elif re.match(r'[а-я]', word):
            if word in rus2eng:
                bot.reply_to(message, rus2eng[word])
            else: 
                bot.send_message(message.chat.id,'Этого термина еще нет')
        else:
